...
#define MRAFT_CHECK_RET_AND_RAISE(ret, func) do { \
      if(ret != 0) { \
...
class Raft {
    ...
    Raft(margo_instance_id mid, raft_id id,
         StateMachine& fsm, Log& log,
...
#define MRAFT_WRAP_CPP_LOG_CALL(func, ...) do { \
    ...
    static int _log_load(struct mraft_log* log,
                         ...
                         size_t* n_entries)
    {
        MRAFT_WRAP_CPP_LOG_CALL(load, term, id, snap, start_index, entries, n_entries);
    }
    ...
    static int _log_set_term(struct mraft_log* log,
                             raft_term term)
    {
         MRAFT_WRAP_CPP_LOG_CALL(set_term, term);
    }

    static int _log_set_vote(struct mraft_log* log,
                             raft_id id)
    {
    ...
    static int _log_truncate(struct mraft_log* log,
                             raft_index index)
    {
...
#define MRAFT_WRAP_CPP_FSM_CALL(func, ...) do {          \
    ...
    static int _fsm_apply(struct raft_fsm *fsm, const struct raft_buffer *buf, void **result)
    {
        MRAFT_WRAP_CPP_FSM_CALL(apply, buf, result);
    }

    static int _fsm_snapshot(struct raft_fsm *fsm, struct raft_buffer *bufs[], unsigned *n_bufs)
    {
...
#define MRAFT_WRAP_C_LOG_CALL(func, ...) do { \
    ...
    {
        MRAFT_WRAP_C_LOG_CALL(load, term, id, snap, start_index, entries, n_entries);
    }
    ...
    void set_vote(raft_id id) override
    {
        MRAFT_WRAP_C_LOG_CALL(set_vote, id);
    }

    void append(const struct raft_entry entries[],
                unsigned n_entries) override
    {
