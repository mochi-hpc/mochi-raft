...
int main(int argc, char** argv)
{
    ...
    /* Initialize RAFT */
    /* We multiply the SSG ID by 2 to test out mraft_get_raft_id*/
    struct raft raft;
    ret = mraft_init(&raft, &raft_io, &raft_fsm, self_id * 2, self_addr);
    margo_assert(mid, ret == 0);
    ...
    raft_id my_raft_id;
    ret = mraft_get_raft_id(&raft, self_addr, &my_raft_id);
    margo_assert(mid, ret == 0);
    margo_assert(mid, my_raft_id == 2 * self_id);
