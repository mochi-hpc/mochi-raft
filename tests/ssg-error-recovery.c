...
parse_args(int argc, char** argv, char** ssg_path, bool* join, raft_id* raft_id)
{
    ...
    while ((opt = getopt_long(argc, argv, "p:jr:", long_options, NULL)) != -1) {
        ...
        default:
            fprintf(stderr,
                    "Usage: %s [--ssg-path ssg_path] [--join] [--raft-id "
                    "raft_id]\n",
                    argv[0]);
            return -1;
        }
...
{
    fprintf(stderr, "[debug] my_membership_update_cb\n");
    ...
    case SSG_MEMBER_JOINED:
        ...
        if (self_raft_id == leader_id) {
            ...
            /* Apply a dummy entry to change the role from raft_spare */
            struct raft_buffer buf = {.base = NULL, .len = 0};
...
int main(int argc, char** argv)
{
    ...
    if (self_raft_id <= 0) {
        ret = -1;
        fprintf(stderr, "Specify --raft-id <ID>, where ID > 0\n");
    }
    if (ret < 0) return -1;
