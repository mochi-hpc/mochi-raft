...
struct mraft_hashmap_kv {
    ssg_member_id_t member_id; /* key */
    raft_id         raft_id;   /* value */
    UT_hash_handle  hh;        /* Required by uthash */
};

struct myfsm {
    char*                    content;
    struct mraft_hashmap_kv* map;
};

static int myfsm_map_put(struct myfsm*   myfsm,
                         ssg_member_id_t member_ssg_id,
                         raft_id         member_raft_id)
{
    struct mraft_hashmap_kv* keyvalue = raft_malloc(sizeof(*keyvalue));

    if (keyvalue == NULL) return -1;

    keyvalue->member_id = member_ssg_id;
    keyvalue->raft_id   = member_raft_id;
    HASH_ADD(hh, myfsm->map, member_id, sizeof(keyvalue->member_id), keyvalue);
    return 0;
}

static int myfsm_map_get(struct myfsm*   myfsm,
                         ssg_member_id_t member_ssg_id,
                         raft_id*        member_raft_id)
{
    struct mraft_hashmap_kv* keyvalue = NULL;
    HASH_FIND(hh, myfsm->map, &member_ssg_id, sizeof(member_ssg_id), keyvalue);

    if (keyvalue == NULL) return -1;

    *member_raft_id = keyvalue->raft_id;
    return 0;
}

static int myfsm_map_remove(struct myfsm* myfsm, ssg_member_id_t member_ssg_id)
{
    struct mraft_hashmap_kv* keyvalue = NULL;
    HASH_FIND(hh, myfsm->map, &member_ssg_id, sizeof(member_ssg_id), keyvalue);
...
myfsm_apply(struct raft_fsm* fsm, const struct raft_buffer* buf, void** result)
{
    ...
    /* If the buffer base starts with a '#', we interpret it as if it was a call
     * to add to the fsm mapping */
    if (buf->len > 0 && ((char*)buf->base)[0] == '#') {
        /* Read member_id and raft_id from the sent buffer */
        ssg_member_id_t member_ssg_id;
        raft_id         member_raft_id;
        sscanf((char*)buf->base, "#%020lu%020llu", &member_ssg_id,
               &member_raft_id);

        /* Add them to the mapping */
        int ret = myfsm_map_put(myfsm, member_ssg_id, member_raft_id);
        if (ret != 0) return ret;
        fprintf(stderr, "[test] Added mapping (%lu -> %llu)\n", member_ssg_id,
                member_raft_id);
        return 0;
    }

    /* If the buffer base starts with a '$', we interpret it as if it was a call
     * to remove from the fsm mapping */
    if (buf->len > 0 && ((char*)buf->base)[0] == '$') {
        /* Read member_id from the sent buffer */
        ssg_member_id_t member_ssg_id;
        sscanf((char*)buf->base, "$%020lu", &member_ssg_id);

        /* Remove it from the mapping */
        int ret = myfsm_map_remove(myfsm, member_ssg_id);
        if (ret != 0) return ret;
        fprintf(stderr, "[test] Removed mapping associated to %lu\n",
                member_ssg_id);
        return 0;
    }
                                    ...
                                    ssg_member_update_type_t update_type)
{
    ...
    case SSG_MEMBER_JOINED:
        ...
        if (self_raft_id == leader_id) {
            ...
            /* Here, the server has been added to the SSG group and to the raft
             * cluster. Now, we add a mapping from ssg_id to raft_id to each
             * process in the raft_cluster */
            char mapping_str[20 + 20 + 1 + 1]; /* 20 for the member_id (ULL), 20
                                                  for the raft_id (ULL), 1 for
                                                  the '#', and 1 for the '\0' */
            snprintf(mapping_str, 42, "#%020lu%020llu", member_id,
                     joiner_raft_id);
            struct raft_buffer buf = {.base = (void*)mapping_str, .len = 42};
            mraft_apply(raft, &buf, 1);
    ...
    case SSG_MEMBER_DIED:
        fprintf(
            ...
            member_id);
        /* The leader must remove the member who left from the raft cluster */
        if (self_raft_id == leader_id) {
            /* Get the raft ID from the FSM mapping */
            fprintf(stderr,
                    "[test] [debug] getting the raft_id of member=%lu\n",
                    member_id);
            raft_id leaver_raft_id;
            ret = myfsm_map_get(myfsm, member_id, &leaver_raft_id);
            margo_assert(mid, ret == 0);

            /* Remove the member from the raft cluster */
            fprintf(stderr,
                    "[test] [debug] removing id=%llu from the raft cluster\n",
                    leaver_raft_id);
            ret = mraft_remove(raft, leaver_raft_id);
            margo_assert(mid, ret == 0);
            fprintf(stderr,
                    "[test] [debug] id=%llu has been removed from the raft "
                    "cluster\n",
                    leaver_raft_id);

            /* Inform all remaining members of the raft cluster to remove the
             * mapping associated to the member that left */
            char mapping_str[20 + 1 + 1]; /* 20 for the member_id (ULL), 1 for
                                             the '$', and 1 for the '\0' */
            snprintf(mapping_str, 22, "$%020lu", member_id);
            struct raft_buffer buf = {.base = (void*)mapping_str, .len = 22};
            mraft_apply(raft, &buf, 1);
        }
        break;
    default:
                             ...
                             struct membership_update_arg* arg)
{
    ...
    ssg_group_config_t config            = {.swim_period_length_ms        = 500,
    ...
    /* Add the mapping of ssg_id to raft_id */
    ssg_member_id_t member_id;
    ssg_get_group_member_id_from_rank(gid, 0, &member_id);
    char mapping_str[20 + 20 + 1 + 1]; /* 20 for the member_id (ULL), 20
                                            for the raft_id (ULL), 1 for
                                            the '#', and 1 for the '\0' */
    snprintf(mapping_str, 42, "#%020lu%020llu", member_id, arg->self_raft_id);
    struct raft_buffer buf = {.base = (void*)mapping_str, .len = 42};
    mraft_apply(arg->raft, &buf, 1);
...
int main(int argc, char** argv)
{
    ...
    const char* leader_addr;
    raft_leader(&raft, &leader_id, &leader_addr);
    margo_trace(mid, "[raft] Leader is %lu, state is %d", leader_id,
                raft_state(&raft));
     ...
     * For now, the process doesn't call ssg_group_leave, we reserve that for
     * when a process actually wants to leave the SSG group */
    ret = ssg_group_destroy(arg.gid);
    margo_assert(mid, ret == SSG_SUCCESS);
    fprintf(stderr, "[test] [debug] SSG group destroyed\n");

    /* Finalize RAFT */
    mraft_close(&raft);
    fprintf(stderr, "[test] [debug] mraft closed\n");

    /* Finalize raft_io backend */
    ret = mraft_io_finalize(&raft_io);
    margo_assert(mid, ret == 0);
    fprintf(stderr, "[test] [debug] mraft_io finalized\n");

    /* Finalize the log */
    mraft_memory_log_finalize(&log);
    fprintf(stderr, "[test] [debug] memory_log finalized\n");

    /* Finalize the state machine */
    free(myfsm.content);
    fprintf(stderr, "[test] [debug] myfsm free\n");

    /* Finalize SSG */
    ret = ssg_finalize();
    margo_assert(mid, ret == SSG_SUCCESS);
    fprintf(stderr, "[test] [debug] ssg finalized\n");

    /* Finalize Margo */
    margo_finalize(mid);
    fprintf(stderr, "[test] [debug] margo finalized\n");
